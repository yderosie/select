...
t_liste 	*voir_touche(t_env *e, t_liste *first)
{
	char	buffer[3];
	t_xy	xy;
	t_liste *place;
...
int main(int argc, char **argv)
{
	...
	first_init_e(&e, argv, argc - 1, max_c(argv + 1));
	clear(&e);
	if ((lst = voir_touche(&e, lst)) == NULL)
		i++;
